  ...
  const stats = await Promise.all(
    (categories || []).map(async (cat) => {
      const { count } = await supabase
        ...
        .select('*', { count: 'exact', head: true })
        .eq('status', 'published')
        .contains('issue_categories', [cat.id]);
      return {
        ...cat,
        count: count || 0,
...
export async function getRecentReports(limit = 10) {
  const { data } = await supabase
    .from('reports')
    .select('id, vehicle_make, vehicle_model, vehicle_year, city, state, issue_categories, created_at')
    .eq('status', 'published')
...
// Full list of individual published reports, newest first — for a public,
// browsable record (not just aggregated stats).
